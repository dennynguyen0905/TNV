...
import { archiveLessonAction, toggleLessonPremiumAction } from "./actions";
...
export function LessonsClient({ initialLessons }: LessonsClientProps) {
  ...
  const handleTogglePremium = (id: string) => {
    ...
    setLessons((prev) =>
      prev.map((l) => (l.id === id ? { ...l, premium: newPremium } : l))
    );
  ...
  const confirmDelete = () => {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) console.error("Archive failed:", result.error);
    });
  };
